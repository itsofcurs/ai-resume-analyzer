...
from .schemas import UserCreate, UserLogin, Token, JobDescriptionCreate, MatchRequest
from .auth import get_password_hash, verify_password, create_access_token, get_current_user, role_required
from .nlp_pipeline import process_resume
from utils.file_parser import extract_text
from .copilot import generate_candidate_summary, analyze_job_fit, generate_interview_questions, detect_fraud
...
async def upload_resume(
    ...
    await manager.broadcast(json.dumps({"event": "processing_start", "filename": file.filename}))
    ...
    await manager.broadcast(json.dumps({"event": "processing_complete", "resume_id": new_resume.id, "name": new_resume.candidate_name}))
    
    return {"message": "Success", "resume_id": new_resume.id, "data": extracted_data}

@app.get("/api/resumes")
async def get_resumes(db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)):
    result = await db.execute(select(Resume).order_by(Resume.created_at.desc()))
    return result.scalars().all()

# --- JOB DESCRIPTION & MATCHING ---
@app.post("/api/jobs")
async def create_job(job: JobDescriptionCreate, db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)):
...
@app.get("/api/jobs")
async def get_jobs(db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)):
    result = await db.execute(select(JobDescription).order_by(JobDescription.created_at.desc()))
    return result.scalars().all()

@app.get("/api/jobs/{job_id}/match")
async def semantic_job_match(job_id: int, db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)):
    # 1. Fetch Job
