...
# ---------------------------------------------------------------------------
from database import mongo_health_check, vector_search, vector_search_ready
from embeddings import embedding_ready, generate_query_embedding
...
from schemas.job_match_schema import FinalATSAnalysisSchema, JobMatchRequestSchema
...
from services.metrics_service import API_LATENCY, API_REQUESTS
...
OPENAPI_TAGS = [
    ...
    {
        "name": "Recruiter ATS",
        "description": "Recruiter-facing ATS scoring and ranking.",
    },
...
async def log_timeout_to_node(
    workflow: str, organizationId: str, duration: int, status: str
):
...
async def _startup() -> None:
    ...
    if settings.redis_url:
        try:
            ...
            app.state.redis_rate_limiter = None
    app.state.concurrency_guard = ConcurrencyGuard(
        default_limit=settings.max_inflight_search
    )
...
async def request_guard(request: Request, call_next):
    ...
    redis_limiter: RedisRateLimiter | None = getattr(
    ...
    internal_key = request.headers.get("x-internal-api-key") or request.headers.get(
        "x-api-key"
    )
    ...
    if rate_limit and not is_internal_auth:
        ...
        if redis_limiter is not None:
            allowed = redis_limiter.allow(
                key, limit=rate_limit["limit"], window_seconds=rate_limit["window"]
            )
            ...
            try:
                if timeout_s:
                    response = await asyncio.wait_for(
                        call_next(request), timeout=timeout_s
                    )
                else:
...
async def workflow_timeout_handler(request: Request, exc: WorkflowTimeoutError):
    return JSONResponse(
        status_code=504,
        content={
            "detail": "Workflow timed out. Please retry.",
...
async def processing_error_handler(request: Request, exc: Exception):
    return JSONResponse(
        status_code=503,
        content={
            "detail": "Processing service unavailable. Please retry.",
...
async def process_resume(
    req: ProcessRequest, background_tasks: BackgroundTasks, request: Request
):
    """
...
async def compare_candidates(req: CompareRequest):
    try:
        logger.info(
            "POST /api/compare — a=%s b=%s", req.candidate_a_id, req.candidate_b_id
        )
...
async def copilot_agent(req: CopilotAgentRequest):
    try:
        ...
        await log_timeout_to_node(
            "AutonomousRecruiter", req.organization_id or "system", 120, "timeout"
        )
...
async def copilot_recruiter(req: CopilotAgentRequest):
    try:
        ...
        await log_timeout_to_node(
            "CopilotRecruiter", req.organization_id or "system", 120, "timeout"
        )
        raise HTTPException(status_code=504, detail="Agent timeout")
    except HTTPException:
        raise
...
async def regenerate_interview(
    req: InterviewRegenerateRequest, background_tasks: BackgroundTasks
):
    try:
...
async def explain_recommendation(req: ExplainabilityRequest):
    ...
    try:
        ...
        API_LATENCY.labels(endpoint="/api/explain/recommendation").observe(
            time.time() - start_time
        )
        API_REQUESTS.labels(
            endpoint="/api/explain/recommendation", status="success"
        ).inc()
        ...
        API_REQUESTS.labels(
            endpoint="/api/explain/recommendation", status="error"
        ).inc()
...
@app.get(
    "/api/memory/candidate/{candidate_id}",
    tags=["Memory"],
    dependencies=[Depends(RateLimiter(times=100, seconds=60))],
)
...
@app.get(
    "/api/memory/recruiter/{recruiter_id}",
    tags=["Memory"],
    dependencies=[Depends(RateLimiter(times=100, seconds=60))],
)
...
async def process_learning_outcome(req: LearningOutcomeRequest):
    ...
    try:
        ...
        API_LATENCY.labels(endpoint="/api/learning/outcome").observe(
            time.time() - start_time
        )
...
async def generate_interview_prep(req: InterviewPrepRequest):
    try:
        logger.info(
            f"POST /api/interview/prep — resume_id='{req.resume_id}' topic='{req.topic}' mode='{req.mode}'"
        )
...
async def evaluate_interview_answers(req: InterviewEvaluateRequest):
    try:
        logger.info(
            f"POST /api/interview/evaluate — resume_id='{req.resume_id}' answers_count={len(req.answers)}"
        )
...
async def generate_analytics_insights(req: AnalyticsInsightsRequest):
    try:
        ...
        result = await _hiring_insights_workflow.run(
            req.organization_id, req.aggregated_stats
        )
        if "error" in result and result["error"]:
            raise HTTPException(status_code=500, detail=result["error"])
        return result
...
@app.post(
    "/api/rediscovery/search", summary="Candidate Rediscovery Engine", tags=["Phase4C"]
)
...
@app.post(
    "/api/interview/live-analysis", summary="Live Interview Copilot", tags=["Phase4C"]
)
...
@app.post(
    "/api/autonomous/run", summary="Run Background Autonomous Agent", tags=["Phase4C"]
)
async def run_autonomous_agent(
    req: AutonomousAgentRunRequest, background_tasks: BackgroundTasks
):
    try:
...
@app.post(
    "/api/copilot/recruiter", summary="Interactive Recruiter Copilot", tags=["Phase4C"]
)
async def run_recruiter_copilot(req: RecruiterCopilotRequest):
    try:
        # Invoke autonomous agent to ensure everything is up to date
        await _autonomous_agent_workflow.run(
            req.candidate_id, req.organization_id, req.job_id, "recruiter_copilot"
        )
        ...
        doc = await collection.find_one(
            {"_id": ObjectId(req.candidate_id), "organizationId": req.organization_id}
        )
        ...
        return {
            "recommendation": doc.get(
                "recommendationReason", "No recommendation generated."
            ),
            "atsScore": doc.get("atsScores", {}).get("overall_score", 0),
            "trustScore": doc.get("fraudAnalysis", {}).get("trustScore", 0),
            "successProbability": doc.get("successPrediction", {}).get(
                "successProbability", 0
            ),
            "risks": doc.get("predictiveHiring", {}).get("risks", []),
            "nextActions": [
                req.recruiter_prompt
            ],  # Echo or generate next actions based on prompt
        }
    except Exception as exc:
        logger.error("POST /api/copilot/recruiter — failed: %s", exc)
        raise HTTPException(status_code=500, detail=str(exc))
...
async def generate_knowledge_graph(
    req: KnowledgeGraphRequest, background_tasks: BackgroundTasks
):
    logger.info(f"POST /api/knowledge-graph/generate — resume_id='{req.resume_id}'")
    background_tasks.add_task(
        _knowledge_graph_workflow.run, req.resume_id, req.organization_id
    )
...
async def analyze_media(
    req: VoiceVideoAnalyzeRequest, background_tasks: BackgroundTasks
):
    logger.info(
        f"POST /api/media/analyze — resume_id='{req.resume_id}' round='{req.round_type}'"
    )
    background_tasks.add_task(
        _voice_video_workflow.run,
        req.resume_id,
        req.organization_id,
        req.round_type,
        req.media_url,
...
async def job_match(req: JobMatchRequestSchema, request: Request, response: Response):
    ...
    try:
        ...
        required = {
            s.strip().lower() for s in (req.required_skills or []) if s and s.strip()
        }
        resume_skills = {
            s.strip().lower()
            for s in (
                (result.parsed_resume.skills if result.parsed_resume else []) or []
            )
            if s and s.strip()
        }
...
async def job_match_v1(
    req: JobMatchRequestSchema, request: Request, response: Response
):
...
@app.get(
    "/api/system/config-safe", summary="Non-sensitive runtime config", tags=["Health"]
)
...
@app.post("/api/copilot/recruiter/alternate", tags=["Phase 4C"])
async def copilot_recruiter_alternate(req: RecruiterCopilotRequest, request: Request):
    try:
        ...
        await wf.run(
            req.candidate_id
        )  # The signature in AutonomousAgentWorkflow might only accept resume_id, let's verify. Let's just pass candidate_id.
        ...
        if candidate and candidate.get("successPrediction"):
            successProbability = candidate.get("successPrediction").get(
                "successProbability", 0
            )
