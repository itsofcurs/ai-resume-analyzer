...
class LLMRouter:
    ...
    def get_llm(cls, task: str) -> Runnable:
        ...
        if task in ["ats_scoring", "candidate_ranking", "ranking", "interview_generation", "interview", "resume_parsing", "copilot"]:
        ...
        if not providers:
            raise RuntimeError("No LLM Providers available! Check API keys and configuration.")
        ...
        for p in providers:
            try:
                ...
                logger.warning(f"Failed to instantiate client for provider {p.provider_name}: {e}")
        ...
        logger.info(f"LLMRouter [{task}]: Configured primary={providers[0].provider_name} with {len(fallbacks)} fallbacks.")
