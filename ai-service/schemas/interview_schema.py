...
class ProjectQuestion(BaseModel):
    ...
    project: str = Field(
        description="The name or description of the project being asked about"
    )
...
class InterviewQuestionsSchema(BaseModel):
    technicalQuestions: List[TechnicalQuestion] = Field(
        description="List of technical questions", default_factory=list
    )
    projectQuestions: List[ProjectQuestion] = Field(
        description="List of project questions", default_factory=list
    )
    behavioralQuestions: List[BehavioralQuestion] = Field(
        description="List of behavioral questions", default_factory=list
    )
    followUpQuestions: List[FollowUpQuestion] = Field(
        description="List of follow-up questions", default_factory=list
    )
