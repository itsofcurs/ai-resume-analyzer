...
class VoiceVideoIntelligenceWorkflow:
    ...
    async def _node_download_media(self, state: VoiceVideoState) -> VoiceVideoState:
        ...
        try:
            ...
            req = requests.get(media_url, stream=True, timeout=60, allow_redirects=False)
    ...
    async def _node_transcribe_media(self, state: VoiceVideoState) -> VoiceVideoState:
        ...
        try:
            ...
            try:
                ...
                collection = await get_mongo_collection("resumes")
                resume = await collection.find_one({"_id": ObjectId(state["resume_id"]), "organizationId": state["organization_id"]})
                if resume and "voiceVideoAnalysis" in resume:
                    for round_data in reversed(resume["voiceVideoAnalysis"]):
                        ...
                        if round_data.get("mediaUrl") == media_url and round_data.get("transcript"):
            ...
            if self.whisper_model and local_media_path and os.path.exists(local_media_path):
                try:
                    segments, info = self.whisper_model.transcribe(local_media_path, beam_size=5)
                    transcript = " ".join([segment.text for segment in segments])
                except Exception as e:
                    logger.warning(f"Whisper transcription failed, falling back to dummy: {e}")
                    transcript = "This is a fallback transcript due to transcription failure."
            else:
            ...
            if local_media_path and local_media_path.startswith(tempfile.gettempdir()):
                try:
                    ...
                    logger.error(f"Failed to clean up temporary file {local_media_path}: {e}")
    ...
    async def _run_llm_node(self, node_name: str, prompt_text: str, state: VoiceVideoState, keys_to_extract: List[str]):
        try:
    ...
    async def _node_communication(self, state: VoiceVideoState) -> VoiceVideoState:
        ...
        return await self._run_llm_node("Communication", prompt, state, ["communication_score", "clarity_score"])
    ...
    async def _node_confidence(self, state: VoiceVideoState) -> VoiceVideoState:
        ...
        return await self._run_llm_node("Confidence", prompt, state, ["confidence_score"])
    ...
    async def _node_professionalism(self, state: VoiceVideoState) -> VoiceVideoState:
        ...
        return await self._run_llm_node("Professionalism", prompt, state, ["professionalism_score"])
    ...
    async def _node_leadership(self, state: VoiceVideoState) -> VoiceVideoState:
        ...
        return await self._run_llm_node("Leadership", prompt, state, ["leadership_presence_score"])
    ...
    async def _node_engagement(self, state: VoiceVideoState) -> VoiceVideoState:
        ...
        return await self._run_llm_node("Engagement", prompt, state, ["engagement_score", "sentiment_score"])
    ...
    async def _node_behavioral(self, state: VoiceVideoState) -> VoiceVideoState:
        ...
        return await self._run_llm_node("Behavioral", prompt, state, ["strengths", "weaknesses", "behavioral_indicators"])
    ...
    async def _node_authenticity(self, state: VoiceVideoState) -> VoiceVideoState:
        ...
        auth = (state.get("confidence_score", 80) * 0.4) + (state.get("sentiment_score", 80) * 0.3) + 25.0
    ...
    async def _node_integrity(self, state: VoiceVideoState) -> VoiceVideoState:
        ...
        return await self._run_llm_node("Integrity", prompt, state, [
    ...
    async def run(self, resume_id: str, organization_id: str, round_type: str = "TECHNICAL", media_url: str = "") -> dict:
