...
class RediscoveryWorkflow:
    def __init__(self):
        ...
        graph.add_conditional_edges("fetch_job", lambda s: "handle_failure" if s.get("error") else "vector_search")
        graph.add_conditional_edges("vector_search", lambda s: "handle_failure" if s.get("error") else "llm_rerank")
        graph.add_conditional_edges("llm_rerank", lambda s: "handle_failure" if s.get("error") else END)
    ...
    async def _node_llm_rerank(self, state: RediscoveryState) -> RediscoveryState:
        try:
            ...
            for m in state["vector_matches"]:
                doc = await collection.find_one({"_id": ObjectId(m["resume_id"]), "organizationId": state["organization_id"]})
                if doc:
                    candidates.append({
                        "id": str(doc["_id"]),
                        ...
                        "success_prob": doc.get("successPrediction", {}).get("successProbability", 50),
                        "risk": doc.get("predictiveHiring", {}).get("flightRisk", 50)
                    })
            ...
            res = await chain.ainvoke({"jd": state["job_description"], "candidates": json.dumps(candidates)})
            parsed = json.loads(clean_json_str(res))
            
            # Merge LLM score with candidate data
            scored = []
            for c in candidates:
                llm_res = next((r for r in parsed.get("results", []) if r["id"] == c["id"]), None)
                rediscovery_score = llm_res["rediscovery_score"] if llm_res else (c["semantic_score"] * 100)
