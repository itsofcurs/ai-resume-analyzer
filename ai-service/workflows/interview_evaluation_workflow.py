...
class InterviewEvaluationWorkflow:
    ...
    async def _emit_event(self, resume_id: str, event_name: str):
        ...
        try:
            ...
            logger.warning(
                f"[INTERVIEW_EVALUATION] Failed to send webhook for {event_name}: {exc}"
            )

    async def _node_load_candidate(
        self, state: InterviewEvaluationState
    ) -> InterviewEvaluationState:
        logger.info(
            f"[INTERVIEW_EVALUATION] Stage 1 - Loading candidate {state['resume_id']}"
        )
        try:
    ...
    async def run(self, resume_id: str, answers: List[Dict[str, str]]) -> dict:
        ...
        try:
            ...
            logger.error(
                f"[INTERVIEW_EVALUATION] Failed to trigger fraud detection automatically: {e}"
            )
