...
class AutonomousRecruiterWorkflow:
    ...
    async def _node_detect_intent(
        self, state: RecruiterCopilotState
    ) -> RecruiterCopilotState:
        ...
        try:
            if intent == "SEARCH_CANDIDATES":
                ...
                filtered = [
                    m
                    for m in matches
                    if str(m.get("organizationId", org_id)) == str(org_id)
                ]
                ...
                docs = (
                    await collection.find(
                        match_query, {"candidateName": 1, "atsScores": 1}
                    )
                    .sort("atsScores.overall_score", -1)
                    .limit(5)
                    .to_list(length=5)
                )
                for d in docs:
                ...
                docs = (
                    await collection.find(
                        match_query, {"candidateName": 1, "fraudAnalysis": 1}
                    )
                ...
                docs = (
                    await collection.find(
                        match_query, {"candidateName": 1, "successPrediction": 1}
                    )
                    ...
                    [
                        {"$match": match_query},
                        {
                            "$group": {
                                "_id": None,
                                "avg_success": {
                                    "$avg": "$successPrediction.successProbability"
                                },
                            }
                        },
                    ]
                ...
                avg_success = (
                    avg_success_docs[0]["avg_success"] if avg_success_docs else 0
                )
                results["analytics"] = {
                ...
                docs = await collection.find(
                    {
                        **match_query,
                        "voiceVideoAnalysis": {"$exists": True, "$not": {"$size": 0}},
                    },
                    {
                        "candidateName": 1,
                        "voiceVideoAnalysis": {"$slice": -1},
                    },  # Get only the latest round
                ).to_list(length=10)
                ...
                sorted_docs = sorted(
                    docs,
                    key=lambda x: x.get("voiceVideoAnalysis", [{}])[0].get(sort_key, 0),
                    reverse=True,
                )
    ...
    async def _node_reasoning_layer(
        self, state: RecruiterCopilotState
    ) -> RecruiterCopilotState:
