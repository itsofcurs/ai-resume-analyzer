...
class AuthenticityWorkflow:
    def __init__(self):
        ...
        builder.add_conditional_edges(
            "load_candidate",
            lambda s: "detect_copy_paste_patterns" if not s.get("error") else END,
            ["detect_copy_paste_patterns", END],
        )
        builder.add_conditional_edges(
            "detect_copy_paste_patterns",
            lambda s: "detect_ai_generated_patterns" if not s.get("error") else END,
            ["detect_ai_generated_patterns", END],
        )
        builder.add_conditional_edges(
            "detect_ai_generated_patterns",
            lambda s: "compare_resume_consistency" if not s.get("error") else END,
            ["compare_resume_consistency", END],
        )
        builder.add_conditional_edges(
            "compare_resume_consistency",
            lambda s: "similarity_analysis" if not s.get("error") else END,
            ["similarity_analysis", END],
        )
        builder.add_conditional_edges(
            "similarity_analysis",
            lambda s: "calculate_authenticity_score" if not s.get("error") else END,
            ["calculate_authenticity_score", END],
        )
        builder.add_conditional_edges(
            "calculate_authenticity_score",
            lambda s: "generate_report" if not s.get("error") else END,
            ["generate_report", END],
        )
        builder.add_conditional_edges(
            "generate_report",
    ...
    async def _node_load_candidate(self, state: AuthenticityState) -> AuthenticityState:
        ...
        try:
            ...
            state["interview_questions"] = [
                qa.get("question") for qa in qa_list if qa.get("question")
            ]
            state["interview_answers"] = [
                qa.get("answer") for qa in qa_list if qa.get("answer")
            ]
    ...
    async def _node_copy_paste(self, state: AuthenticityState) -> AuthenticityState:
        ...
        try:
            ...
            raw = await ainvoke_with_retry(
                chain, {"answers_json": json.dumps(state["interview_answers"])}
            )
    ...
    async def _node_calculate_score(
        self, state: AuthenticityState
    ) -> AuthenticityState:
        if state.get("error"):
            return state
