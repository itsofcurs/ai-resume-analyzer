...
class KnowledgeGraphWorkflow:
    def __init__(self):
        ...
        builder.add_conditional_edges(
            "load_candidate",
            lambda s: "build_networks" if not s.get("error") else END,
            ["build_networks", END],
        )
        builder.add_conditional_edges(
            "build_networks",
            lambda s: "detect_hidden_talents" if not s.get("error") else END,
            ["detect_hidden_talents", END],
        )
        builder.add_conditional_edges(
            "detect_hidden_talents",
            lambda s: "cluster_candidate" if not s.get("error") else END,
            ["cluster_candidate", END],
        )
        builder.add_conditional_edges(
            "cluster_candidate",
            lambda s: "find_similar_candidates" if not s.get("error") else END,
            ["find_similar_candidates", END],
        )
        builder.add_conditional_edges(
            "find_similar_candidates",
            lambda s: "generate_graph_summary" if not s.get("error") else END,
            ["generate_graph_summary", END],
        )
        builder.add_conditional_edges(
    ...
    async def _node_load_candidate(
        ...
        emit_socket_event(
            "GRAPH_ANALYZING",
            {"resumeId": state["resume_id"], "status": "Loading Data"},
        )
        try:
    ...
    async def _node_build_networks(
        self, state: KnowledgeGraphState
    ) -> KnowledgeGraphState:
        if state.get("error"):
            return state
        emit_socket_event(
            "GRAPH_ANALYZING",
            {"resumeId": state["resume_id"], "status": "Building Networks"},
        )
        try:
            ...
            raw = await chain.ainvoke(
                {"skills": skills, "experience": experience[:2000]}
            )
    ...
    async def _node_detect_hidden_talents(
        self, state: KnowledgeGraphState
    ) -> KnowledgeGraphState:
        if state.get("error"):
            return state
        emit_socket_event(
            "GRAPH_ANALYZING",
            {"resumeId": state["resume_id"], "status": "Detecting Hidden Talents"},
        )
        try:
    ...
    async def _node_cluster_candidate(
        self, state: KnowledgeGraphState
    ) -> KnowledgeGraphState:
        if state.get("error"):
            return state
        emit_socket_event(
            "GRAPH_CLUSTERING",
            {"resumeId": state["resume_id"], "status": "Clustering Candidate"},
        )
        try:
            ...
            state["candidate_cluster"] = data.get(
                "candidateCluster", "Full Stack Engineer"
            )
        except Exception as e:
    ...
    async def _node_find_similar_candidates(
        self, state: KnowledgeGraphState
    ) -> KnowledgeGraphState:
        if state.get("error"):
            return state
        emit_socket_event(
            "GRAPH_ANALYZING",
            {"resumeId": state["resume_id"], "status": "Finding Similar Candidates"},
        )
        try:
            collection = get_mongo_collection()
            ...
            base_success = (
                ...
                .get("successProbability", 0)
            )
            base_skill = (
                state["resume_data"]
                .get("skillGraph", {})
                .get("overallTechnicalScore", 0)
            )
            ...
            for peer in peers:
                ...
                peer_success = peer.get("successPrediction", {}).get(
    ...
    async def _node_generate_graph_summary(
        self, state: KnowledgeGraphState
    ) -> KnowledgeGraphState:
        if state.get("error"):
            return state
        try:
            ...
            score = 50
            if state["candidate_cluster"] != "Unknown":
                score += 10
    ...
    async def _node_persist(self, state: KnowledgeGraphState) -> KnowledgeGraphState:
        ...
        emit_socket_event(
            "GRAPH_ANALYZING",
            {"resumeId": state["resume_id"], "status": "Saving Knowledge Graph"},
        )
        try:
            collection = get_mongo_collection()
            ...
            await collection.update_one(
                {"_id": ObjectId(state["resume_id"])}, {"$set": {"knowledgeGraph": doc}}
            )
