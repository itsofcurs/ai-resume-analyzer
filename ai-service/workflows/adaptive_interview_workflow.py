...
class AdaptiveInterviewWorkflow:
    def __init__(self):
        ...
        builder.add_conditional_edges("evaluate_answer", lambda s: "determine_direction" if not s.get("error") else END, ["determine_direction", END])
        builder.add_conditional_edges("determine_direction", lambda s: "generate_question" if not s.get("error") else END, ["generate_question", END])
        builder.add_edge("generate_question", END)
        
        self._graph = builder.compile()

    async def _node_evaluate_answer(self, state: AdaptiveInterviewState) -> AdaptiveInterviewState:
        ...
        try:
            ...
            if not history:
                ...
                state["last_answer_evaluation"] = {"score": 100, "feedback": "Initial", "missing_concepts": []}
                return state
            ...
            if not last_answer:
                state["last_answer_evaluation"] = {"score": 0, "feedback": "No answer provided.", "missing_concepts": ["Everything"]}
                return state
    ...
    async def _node_determine_direction(self, state: AdaptiveInterviewState) -> AdaptiveInterviewState:
        if state.get("error"): return state
    ...
    async def _node_generate_question(self, state: AdaptiveInterviewState) -> AdaptiveInterviewState:
        if state.get("error"): return state
        ...
        try:
            ...
            raw = await ainvoke_with_retry(chain, {
                ...
                "history": history_str,
                "score": eval_data.get("score", 50),
                "missing_concepts": json.dumps(eval_data.get("missing_concepts", [])),
                "direction": state.get("direction", "explore_new")
            })
            
            data = json.loads(clean_json_str(raw))
            state["next_question"] = data.get("next_question", "Could you elaborate on your experience?")
        except Exception as e:
            state["error"] = str(e)
        return state

    async def run(self, current_topic: str, conversation_history: List[dict], resume_id: str = "test", organization_id: str = "org") -> dict:
