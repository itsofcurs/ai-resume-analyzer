...
class FraudDetectionWorkflow:
    def __init__(self):
        ...
        builder.add_conditional_edges(
            "load_candidate",
            lambda s: "analyze_skills" if not s.get("error") else END,
            ["analyze_skills", END],
        )
        builder.add_conditional_edges(
            "analyze_skills",
            lambda s: "analyze_projects" if not s.get("error") else END,
            ["analyze_projects", END],
        )
        builder.add_conditional_edges(
            "analyze_projects",
            lambda s: "analyze_timeline" if not s.get("error") else END,
            ["analyze_timeline", END],
        )
        builder.add_conditional_edges(
            "analyze_timeline",
            lambda s: "detect_contradictions" if not s.get("error") else END,
            ["detect_contradictions", END],
        )
        builder.add_conditional_edges(
            "detect_contradictions",
            lambda s: "calculate_score" if not s.get("error") else END,
            ["calculate_score", END],
        )
        builder.add_conditional_edges(
            "calculate_score",
    ...
    async def _node_load_candidate(
        self, state: FraudDetectionState
    ) -> FraudDetectionState:
    ...
    async def _node_analyze_skills(
        self, state: FraudDetectionState
    ) -> FraudDetectionState:
    ...
    async def _node_analyze_projects(
        self, state: FraudDetectionState
    ) -> FraudDetectionState:
    ...
    async def _node_analyze_timeline(
        self, state: FraudDetectionState
    ) -> FraudDetectionState:
        ...
        try:
            ...
            state["timeline_analysis"] = await ainvoke_with_retry(
                chain, {"resume_json": json.dumps(state.get("resume_data", {}))}
            )
        except Exception as e:
            state["error"] = str(e)
            await self._emit_event(state["resume_id"], "FRAUD_FAILED")
    ...
    async def _node_detect_contradictions(
        self, state: FraudDetectionState
    ) -> FraudDetectionState:
        ...
        try:
            ...
            raw_response = await ainvoke_with_retry(
                chain,
                {
                    ...
                    "voice_video_json": json.dumps(
                        state.get("voice_video_analysis", []), default=str
                    ),
                },
            )
