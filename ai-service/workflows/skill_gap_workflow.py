...
class SkillGapWorkflow:
    def __init__(self):
        ...
        graph.add_node(
            "analyze_strengths_weaknesses", self._node_analyze_strengths_weaknesses
        )
        ...
        graph.add_node(
            "generate_development_plan", self._node_generate_development_plan
        )
    ...
    async def _node_analyze_strengths_weaknesses(
        self, state: SkillGapState
    ) -> SkillGapState:
        if state.get("error"):
            return state
    ...
    async def _node_calculate_readiness(self, state: SkillGapState) -> SkillGapState:
        ...
        try:
            ...
            state["skill_gap_analysis"] = {
                "estimatedJobReadiness": data.get("estimatedJobReadiness", "")
            }
        except Exception as e:
            state["error"] = str(e)
        return state

    async def _node_generate_development_plan(
        self, state: SkillGapState
    ) -> SkillGapState:
        if state.get("error"):
            return state
            ...
            sg.update(
                {
                    ...
                    "missingSkills": state.get("skill_gaps", {}).get(
                        "missingSkills", []
                    ),
                    "missingTechnologies": state.get("skill_gaps", {}).get(
                        "missingTechnologies", []
                    ),
                    ...
                    "recommendedCertifications": data.get(
    ...
    async def _node_persist(self, state: SkillGapState) -> SkillGapState:
        ...
        try:
            ...
            try:
                from workflows.predictive_hiring_workflow import (
                    PredictiveHiringWorkflow,
                )
                ...
                logger.error(
                    f"[SKILL_GAP] Failed to trigger predictive hiring automatically: {e}"
                )
