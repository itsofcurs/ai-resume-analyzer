...
class SuccessPredictionWorkflow:
    def __init__(self):
        ...
        builder.add_node("predict_retention_leadership", self._node_retention_leadership)
        ...
        # Edges
        builder.add_conditional_edges("load_candidate", lambda s: "analyze_behavioral_signals" if not s.get("error") else END, ["analyze_behavioral_signals", END])
        builder.add_conditional_edges("analyze_behavioral_signals", lambda s: "analyze_learning_agility" if not s.get("error") else END, ["analyze_learning_agility", END])
        builder.add_conditional_edges("analyze_learning_agility", lambda s: "predict_retention_leadership" if not s.get("error") else END, ["predict_retention_leadership", END])
        builder.add_conditional_edges("predict_retention_leadership", lambda s: "analyze_cultural_fit" if not s.get("error") else END, ["analyze_cultural_fit", END])
        builder.add_conditional_edges("analyze_cultural_fit", lambda s: "generate_success_prediction" if not s.get("error") else END, ["generate_success_prediction", END])
        builder.add_conditional_edges("generate_success_prediction", lambda s: "persist_results" if not s.get("error") else END, ["persist_results", END])
    ...
    async def _node_load_candidate(self, state: SuccessPredictionState) -> SuccessPredictionState:
    ...
    async def _node_behavioral(self, state: SuccessPredictionState) -> SuccessPredictionState:
        if state.get("error"): return state
        try:
            llm = LLMRouter.get_llm("interview")
            chain = BEHAVIORAL_SIGNALS_PROMPT | llm | StrOutputParser()
            state["behavioral_signals"] = await ainvoke_with_retry(chain, {
    ...
    async def _node_agility(self, state: SuccessPredictionState) -> SuccessPredictionState:
        if state.get("error"): return state
        try:
            llm = LLMRouter.get_llm("interview")
    ...
    async def _node_retention_leadership(self, state: SuccessPredictionState) -> SuccessPredictionState:
        if state.get("error"): return state
        try:
            llm = LLMRouter.get_llm("interview")
    ...
    async def _node_cultural_fit(self, state: SuccessPredictionState) -> SuccessPredictionState:
        if state.get("error"): return state
        try:
            llm = LLMRouter.get_llm("interview")
    ...
    async def _node_generate(self, state: SuccessPredictionState) -> SuccessPredictionState:
        if state.get("error"): return state
        try:
            llm = LLMRouter.get_llm("interview")
    ...
    async def _node_persist(self, state: SuccessPredictionState) -> SuccessPredictionState:
        if state.get("error"): return state
        try:
            prediction = {
