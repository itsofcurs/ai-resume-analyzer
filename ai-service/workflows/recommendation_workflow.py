...
class RecommendationWorkflow:
    ...
    async def _node_generate_embedding(
    ...
    async def _node_semantic_search(
        self, state: RecommendationState
    ) -> RecommendationState:
        ...
        try:
            ...
            matches = await vector_search(
                state["query_vector"], top_k=state["top_k"] * 3
            )
    ...
    async def _node_rank_candidates(
        self, state: RecommendationState
    ) -> RecommendationState:
        ...
        try:
            ...
            for match in matches:
                ...
                final_score = (
                    (overall_ats * 0.35)
                    + (scaled_semantic * 0.25)
                    + (skill_score * 0.15)
                    + (exp_score * 0.1)
                    + (overall_tech_score * 0.15)
                )
                ...
                if graph_score > 75:
                    graph_bonus += 5
                if hidden_talents:
                    graph_bonus += len(hidden_talents) * 2
                ...
                if voice_video_analysis and len(voice_video_analysis) > 0:
                    ...
                    if (
                        latest_vv.get("communicationScore", 0) > 80
                        and latest_vv.get("leadershipPresenceScore", 0) > 80
                        and latest_vv.get("interviewIntegrityScore", 0) > 80
                    ):
    ...
    async def _node_handle_failure(
        self, state: RecommendationState
    ) -> RecommendationState:
