...
class AutonomousAgentWorkflow:
    ...
    async def _node_process_ats(
        self, state: BackgroundAgentState
    ) -> BackgroundAgentState:
        try:
            logger.info(
                f"[BACKGROUND AGENT] Running ATS Workflow for {state['resume_id']}"
            )
            wf = ResumeWorkflow()
    ...
    async def _node_process_fraud(
        self, state: BackgroundAgentState
    ) -> BackgroundAgentState:
        try:
            logger.info(
                f"[BACKGROUND AGENT] Running Fraud Workflow for {state['resume_id']}"
            )
    ...
    async def _node_process_success(
        self, state: BackgroundAgentState
    ) -> BackgroundAgentState:
        try:
            logger.info(
                f"[BACKGROUND AGENT] Running Success Workflow for {state['resume_id']}"
            )
    ...
    async def _node_process_risk(
        self, state: BackgroundAgentState
    ) -> BackgroundAgentState:
        try:
            logger.info(
                f"[BACKGROUND AGENT] Running Predictive Risk Workflow for {state['resume_id']}"
            )
    ...
    async def _node_process_jd_match(
        self, state: BackgroundAgentState
    ) -> BackgroundAgentState:
        try:
            if state.get("job_id"):
                logger.info(
                    f"[BACKGROUND AGENT] Running JD Match Workflow for {state['resume_id']} vs {state['job_id']}"
                )
                ...
                if job:
                    ...
                    await wf.run(
                        job.description, top_k=10
                    )  # Updates recommendationScore in Mongo
    ...
    async def _node_finalize(self, state: BackgroundAgentState) -> BackgroundAgentState:
        logger.info(
            f"[BACKGROUND AGENT] Finalizing background run for {state['resume_id']}"
        )
        try:
            collection = get_mongo_collection()
            # Mark processing as fully complete
            await collection.update_one(
                {"_id": ObjectId(state["resume_id"])}, {"$set": {"status": "PROCESSED"}}
            )
    ...
    async def run(
        ...
        return {
            "status": "completed",
            "results": final_state.get("intermediate_results"),
