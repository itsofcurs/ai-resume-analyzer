...
async def run_e2e():
    ...
    resume = await collection.find_one({"status": "PROCESSED", "fraudAnalysis": {"$exists": True}, "skillGapAnalysis": {"$exists": True}})
    
    if not resume:
        print("❌ No suitable resume found in DB for testing. Please upload a resume first.")
        return
