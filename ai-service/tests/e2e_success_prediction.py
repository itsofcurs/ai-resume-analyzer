...
async def test_success_prediction_e2e():
    ...
    if not sample_resume:
        pytest.skip(
            "No processed resume found in the database. Run previous pipeline steps first."
        )
    ...
    for word in protected_words:
        assert (
            word not in summary_lower
        ), f"Safety Violation: found protected characteristic '{word}' in prediction"
